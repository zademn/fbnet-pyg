...
sys.path.append(os.path.join(
    proj_dir, "..", "utils/Pointnet2.PyTorch/pointnet2"))
...
class point_shuffler(nn.Module):
    ...
    def forward(self, inputs: "(B, channel_num, N)"):
        if self.scale == 1:
            ...
            ou = ou.reshape([B, N*self.scale, C//self.scale]
                            ).permute([0, 2, 1])
...
class NodeShuffle(nn.Module):
    ...
    def __init__(self, input_channel, output_channel, neighbor_num=16, scale=2):
        ...
        self.edge_conv = EdgeConv(
            input_channel, input_channel*self.edge_scale, neighbor_num)
        self.mlp = MLP_CONV(in_channel=input_channel*self.edge_scale,
                            layer_dims=[output_channel*scale], bn=True)
...
class EdgeConv(torch.nn.Module):
    ...
    def forward(self, inputs):
        ...
        if self.num_neigh is not None:
            ...
            central_feat = inputs.unsqueeze(
                dim=3).repeat(1, 1, 1, self.num_neigh)
        else:
            central_feat = torch.zeros(
                batch_size, dims, num_points, 1).to(inputs.device)
...
class AdaptGraphPooling(nn.Module):
    ...
    def forward(self, vertices, feature_map):
        ...
        key_point_idx = query_knn(self.neighbor_num, vertices.transpose(
            2, 1).contiguous(), key_point.transpose(2, 1).contiguous(), include_self=True)
        ...
        new_feture_weight = sample_weight[:, 3:, :, :]

        group_feat = group_feat + pos_embedding  #
        new_feat = einsum('b c i j, b c i j -> b c i',
                          new_feture_weight, group_feat)
        new_point = einsum('b c i j, b c i j -> b c i',
                           new_xyz_weight, group_point)
...
class CrossTransformer(nn.Module):
    ...
    def forward(self, pcd, feat, pcd_feadb, feat_feadb):
        ...
        key_point_idx = query_knn(self.n_knn, fusion_pcd.transpose(
            2, 1).contiguous(), key_point.transpose(2, 1).contiguous(), include_self=True)
        ...
        group_feat = group_feat + pos_embedding  #
        refined_feat = einsum('b c i j, b c i j -> b c i',
                              sample_weight, group_feat)
...
class FBAC_BLOCK(nn.Module):
    ...
    def forward(self, pcd, pcd_next, feat_next, cycle=0):
        ...
        feat = self.ext(pcd)
        feat = self.mlp(torch.cat([feat, torch.max(feat, 2, keepdim=True)[
                        0].repeat((1, 1, feat.size(2)))], 1))
            ...
            for idx, upper in enumerate(self.uppers):
                if i == 0:
                    ...
                    if idx > 0:
                        ...
                        # (B, num_pc, 3)
                        pcd = pcd.permute(0, 2, 1).contiguous()
                        pcd = fps_subsample(torch.cat([pcd, partial], 1), npoints).permute(
                            0, 2, 1).contiguous()
                    ...
                    if idx == 0:
                        ...
                        # (B, num_pc, 3)
                        pcd = pcd.permute(0, 2, 1).contiguous()
                        pcd = fps_subsample(torch.cat([pcd, partial], 1), self.num_p0).permute(
                            0, 2, 1).contiguous()
...
class Model(nn.Module):
    def __init__(self, size_z=128, global_feature_size=1024):
        ...
        cycle_num = 3
        self.refine = Feedback_RefinementNet(
            num_p0=num_p0, up_factors=up_factors, cycle_num=cycle_num, return_all_res=True)
    ...
    def forward(self, x, gt=None, prefix="test"):
        ...
        fine = res_pcds[-1]
        return coarse_pcd, res_pcds, fine
